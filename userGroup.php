...
<body>
    ...
    <?php
        ...
        $query = "SELECT * FROM Group_Members WHERE memberid='".$_SESSION['userid']."' ORDER BY groupid";
        ...
        if($numOfAccs == 0)
            ...
            while($row = mysql_fetch_array($result, MYSQL_NUM))
                print("<tr>
                        <td>".$row[2]."</td>
                        <td>".$row[1]."</td>
                        <td><a href='usersInGroup.php?userid=".$row[1]."&group=".$row[2]."'>Check Group</a></td>
                        <td><a href='rmFromGroup.php?userid=".$row[1]."&group=".$row[2]."'>Leave Group</a></td></tr>");
	        }
        print   "</table>";
    ?>
    </div>
